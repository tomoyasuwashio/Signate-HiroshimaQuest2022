...
def make_in_out(
    start_date, end_date, waterlevel, tidelevel, rainfall, waterlevel_stations
):
    stations = set(
        waterlevel_stations[waterlevel_stations["評価対象"] == 1]["観測所名称"])
    in_all_data = {}
    ...
    out_all_data = out_all_data[
        (~out_all_data["value"].isin(["M", "*", "-", "--", "**"]))
        & (~out_all_data["value"].isna())
    ]
...
def main():
    ...
    waterlevel_stations = pd.read_csv(
        os.path.join(data_dir, "waterlevel", "stations.csv")
    )
    in_all_data, out_all_data = make_in_out(
        start_date, end_date, waterlevel, tidelevel, rainfall, waterlevel_stations
    )
    ...
    if columns != {"hour", "station", "value"}:
        print(
            "Invalid data name: {},  Excepted name: {}".format(
                columns, {"hour", "station", "value"}
            )
        )
        return None
    ...
    result = pd.merge(predictions, out_all_data,
                      on=("date", "hour", "station"))

    # compute RMSE
    print("\nRMSE:", np.sqrt(
        ((result["value_x"] - result["value_y"]) ** 2).mean()))
